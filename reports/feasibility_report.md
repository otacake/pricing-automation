...
## 上位者向け説明（意思決定の根拠）
本レポートの目的は「IRR閾値0.06を満たすために必要な保険料倍率の範囲」を数値で特定し、制約違反の有無を確認したうえで、付加保険料設計の意思決定に使える状態へ整理することである。今回の走査では全モデルポイントでmin_rが見つかり、r_end=1.05時点の制約違反は0件であったため、現行の制約設定（irr_hard=0.0、loading_surplus_hard_ratio=-0.10、premium_to_maturity_hard_max=1.05、nbv_hard=0.0）を維持したまま付加保険料を設計可能である。

KPIの最悪値は min IRR=-0.0986235910327119、min NBV=-70191.17194549802、min loading_surplus_ratio=-0.09098095313360245、max premium_to_maturity=1.05、not found=0件である。max premium_to_maturityが1.05で上限に一致するため、r_endに張り付く2点（male_age50_term20, female_age60_term10）は監視対象として扱い、保険料倍率を引き上げる判断を行う場合はこの2点の感度を優先的に確認する。

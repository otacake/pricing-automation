...
def _plot_cashflow_by_profit_source(agg: pd.DataFrame, out_path: Path) -> Path:
    ...
    ax.plot(years, agg["net_cf"], color="#111111", linewidth=2.0, label="Net CF")
    ax.set_title("Yearly Cashflow by Profit Source (All Model Points)")
...
def _plot_annual_premium_by_model_point(summary_df: pd.DataFrame, out_path: Path) -> Path:
    plt = _require_matplotlib()
    out_path.parent.mkdir(parents=True, exist_ok=True)
...
def _build_markdown_report(
    *,
    ...
    lines: list[str] = []
    lines.append(
        f"# Feasibility Report ({config_path.name}, generated {datetime.now(timezone.utc).date().isoformat()})"
    )
    lines.append("")
    ...
    lines.append("")
    lines.append("## Pricing Recommendation (P by Model Point)")
    ...
    for row in constraint_rows:
        status = "PASS" if bool(row["all_ok"]) else "FAIL"
        lines.append(
            f"|{row['constraint']}|{float(row['min_gap']):.6f}|{row['worst_model_point']}|{status}|"
        )
    lines.append("")
    lines.append("## Loading Formula and Coefficients")
    lines.append("- `gross_rate = (net_rate + alpha / a + beta) / (1 - gamma)`")
    if params is None:
        lines.append("- Loading mode: fixed `loading_alpha_beta_gamma`.")
    else:
        lines.append("- Loading mode: `loading_parameters`.")
    ...
    lines.append("")
    lines.append("## Per-model-point alpha/beta/gamma Calculations (with intermediate steps)")
    ...
    lines.append("")
    lines.append("## Yearly Cashflow by Profit Source")
    lines.append(
        f"![Yearly Cashflow by Profit Source]({cashflow_rel})"
    )
    lines.append("")
    lines.append("![Annual Premium by Model Point](" + premium_rel + ")")
    lines.append("")
    ...
    lines.append(
        ...
        f"{config_path.as_posix()} --out reports/executive_pricing_deck.pptx --md-out reports/feasibility_report.md"
    )
    lines.append("```")
    return "\n".join(lines)
...
def _write_executive_pptx(
    *,
    ...
    add_title(
        slide,
        "Executive Summary",
        f"Config: {config_path.name} | Generated: {datetime.now(timezone.utc).date().isoformat()}",
    )
    bullet_box = slide.shapes.add_textbox(Inches(0.7), Inches(1.5), Inches(11.8), Inches(4.8))
    tf = bullet_box.text_frame
    tf.word_wrap = True
    bullets = [
        f"Recommendation: approve pricing table P (annual premium) shown in Slide 2.",
        ...
        f"All quantitative claims are reproducible from out/run_summary_executive.json and out/feasibility_deck_executive.yaml.",
    ]
    tf.text = bullets[0]
    ...
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    add_title(slide, "Pricing Recommendation", "Annual Premium P by Model Point")
    table_rows = len(summary_df) + 1
    table = slide.shapes.add_table(table_rows, 6, Inches(0.4), Inches(1.5), Inches(8.1), Inches(4.7)).table
    headers = ["Model Point", "Annual P", "Monthly P", "IRR", "NBV", "PTM"]
